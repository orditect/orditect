...
The Orditect hot/cold separation contract: the hot path (task state,
semaphore, quota) is one projection, the cold path (trace bundle) is
another. In production the hot path is pinned to Redis:

    TaskRedisDB / AsyncLeaseSemaphore (via LimiterRegistry) / AdmissionQuotaRedisDB

This module provides in-memory implementations of the same duck-typed
protocols so the full governance loop runs with no external services.
Swapping these three doubles for the Redis-backed implementations
...
class InMemoryTaskStorage:
    """In-memory task store matching the flow TaskStorageProtocol.

    Also implements reopen_task (new-generation opening) which
    RecoveryService requires for rerun.
    """
    ...
    async def get_task(self, task_id: str) -> dict[str, Any]:
        # Storage contract: a missing task returns {} and callers check
        # for emptiness themselves (aligned since v0.1.4).
    ...
    async def reopen_task(
        ...
        *,
        ...
        """Open a new execution generation for a terminal task (T3-safe)."""
