...
class PipelineTask(BaseBackEndTask):
    ...
    def __init__(self, storage, orchestrator: TaskOrchestrator, llm, fail_flags) -> None:
        super().__init__(storage)
    ...
    async def execute(self, task_id: str, **kwargs) -> dict:
        ...
        await self._orchestrator.wait_terminal(collect, timeout=10)

        analyze = await self._orchestrator.submit(
            AnalyzeTask(self.storage, self._llm), task_id="analyze"
        )
        await self._orchestrator.wait_terminal(analyze, timeout=10)

        report = await self._orchestrator.submit(
            ReportTask(self.storage, self._llm, self._fail_flags),
            task_id="report",
        )
        record = await self._orchestrator.wait_terminal(report, timeout=10)
        # First run: the report node fails; the root still completes and
        # records the outcome (orchestration semantics stay with the caller).
        return {"report_status": record["status"]}


def make_task_factory(storage, llm, fail_flags):
    """Build the task_factory required by RecoveryService.

    The framework deliberately does not know how to reconstruct a task
    instance from a task_id — that is business semantics (mechanism to the
    framework, semantics to the business).
    """
